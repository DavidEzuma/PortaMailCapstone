...
const float WHEEL_BASE = 0.20;     // Meters
const float TICKS_PER_REV = 1500.0; 

// --- SAFETY LIMITS ---
// 2 MPH ~= 0.894 m/s. We round down slightly for safety.
...
void setMotor(int pwm_pin, int in1_pin, int in2_pin, float speed) {
  // Map speed (-MAX_SPEED to +MAX_SPEED) to PWM (0-255)
...
void subscription_callback(const void * msgin) {
  ...
  // 1. Clamp Linear Speed to Max (2 MPH)
...
void setup() {
  ...
  // --- CRITICAL: CLOCK SYNC ---
  // Attempts to sync time with the Agent (Raspberry Pi)
  // This ensures odometry timestamps match the rest of the ROS network.
